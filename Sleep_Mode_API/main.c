...
void Config_Set()
{
    ...
    SysCtlPeripheralSleepEnable(SYSCTL_PERIPH_GPIOF); // We won't let GPIOF to sleep mode with this function
    SysCtlDeepSleepPowerSet(SYSCTL_FLASH_LOW_POWER | SYSCTL_SRAM_LOW_POWER); // With this mode, We can use more our mcu to low power.
    SysCtlPeripheralClockGating(true);
    SysCtlSleep(); // This Function will start to Sleep Mode

    /* Don't forget to use Delay or if we want to close sleep mode again, it will be really hard.
